...
//	@Query(value = "SELECT COUNT(*) FROM BCS_ACTION_USER_REWARD_CARD_POINT_DETAIL WHERE REFERENCE_ID = ?1", nativeQuery = true)
	...
	@Query(value = "SELECT COUNT(*) FROM BCS_ACTION_USER_REWARD_CARD_POINT_DETAIL WHERE REFERENCE_ID = ?1 AND (POINT_GET_TIME BETWEEN ?2 AND ?3)", nativeQuery = true)
	public Integer countRewardCardPointNumByRewardCardIdAndDate(String rewardCardId, Date startDate, Date endDate);

	@Query(value = "SELECT TOP 1 * FROM BCS_ACTION_USER_REWARD_CARD_POINT_DETAIL WHERE PROCESS_ID = ?", nativeQuery = true)
	ActionUserRewardCardPointDetail findOneByProcessId(String processId);

	@Modifying
    @Transactional
	@Query(value = "DELETE FROM BCS_ACTION_USER_REWARD_CARD_POINT_DETAIL WHERE PROCESS_ID = ?1", nativeQuery = true)
