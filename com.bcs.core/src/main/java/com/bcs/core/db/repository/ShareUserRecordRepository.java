...
public interface ShareUserRecordRepository extends EntityRepository<ShareUserRecord, String>{
    ...
    @Query(value = "SELECT " 
	    				+ "BCS_SHARE_USER_RECORD.UID uid, " 
	    				+ "BCS_SHARE_USER_RECORD.MODIFY_TIME share_time, " 
	    				+ "COUNT(BCS_SHARE_CAMPAIGN_CLICK_TRACING.CLICK_TRACING_ID) share_amount, " 
	    				+ "SUM(IIF(BCS_LINE_USER.CREATE_TIME > BCS_SHARE_CAMPAIGN_CLICK_TRACING.SHARED_TIME, 1, 0)) new_amount " 
    				+ "FROM BCS_SHARE_USER_RECORD " 
			    	+ "JOIN BCS_SHARE_CAMPAIGN_CLICK_TRACING ON (BCS_SHARE_CAMPAIGN_CLICK_TRACING.SHARE_USER_RECORD_ID = BCS_SHARE_USER_RECORD.SHARE_USER_RECORD_ID) " 
			    	+ "JOIN BCS_SHARE_DONATOR_RECORD ON (BCS_SHARE_CAMPAIGN_CLICK_TRACING.UID = BCS_SHARE_DONATOR_RECORD.DONATOR_UID AND BCS_SHARE_CAMPAIGN_CLICK_TRACING.SHARE_USER_RECORD_ID = BCS_SHARE_DONATOR_RECORD.SHARE_USER_RECORD_ID) " 
			    	+ "JOIN BCS_LINE_USER ON (BCS_SHARE_CAMPAIGN_CLICK_TRACING.UID = BCS_LINE_USER.MID) " 
			    	+ "WHERE " 
				    	+ "BCS_SHARE_USER_RECORD.MODIFY_TIME >= ?1 "
				    	+ "AND BCS_SHARE_USER_RECORD.MODIFY_TIME < ?2 "
				    	+ "AND BCS_SHARE_USER_RECORD.CAMPAIGN_ID = ?3 "
				    	+ "AND BCS_SHARE_USER_RECORD.CUMULATIVE_COUNT >= (SELECT SHARE_TIMES FROM BCS_SHARE_CAMPAIGN WHERE BCS_SHARE_CAMPAIGN.CAMPAIGN_ID = BCS_SHARE_USER_RECORD.CAMPAIGN_ID) " 
			    	+ "GROUP BY " 
				    	+ "BCS_SHARE_USER_RECORD.UID, "
				    	+ "BCS_SHARE_USER_RECORD.MODIFY_TIME ", nativeQuery = true)
