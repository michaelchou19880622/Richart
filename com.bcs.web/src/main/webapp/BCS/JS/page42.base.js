...
$(function(){
	$('.btn_add').click(function(){
 		window.location.href=bcs.bcsContextPath + '/edit/rewardCardCreatePage?actionType=Create&from=active';
	});
	$('.ActiveBtn').click(function(){
 		window.location.href=bcs.bcsContextPath +'/edit/rewardCardListPage';
	});
	$('.DisableBtn').click(function(){
 		window.location.href=bcs.bcsContextPath +'/edit/rewardCardListDisablePage';
	});
	...
	$('.csvFile').on("change", function(e) {
		...
		if (input.files && input.files[0]) {
    		...
    		$.ajax({
    			...
    			var successString='';
    			successString+="上傳集點名單成功，已完成手動補點，已完成 "+response.count+" 筆資料。\n";
//    			if(response.overPointCount!=0){
//    				successString+="--------------------------\n";
//    				successString+="點數已滿，所以不予以補點的資料共"+response.overPointCount+"筆\n";
//    				successString+="不予以補點的UID為下列所示：\n";
//    				for(var index in response.overPointUsers){
