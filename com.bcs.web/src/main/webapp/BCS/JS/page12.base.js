...
$(function(){
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/interactiveResponseCreatePage?from=interactiveResponsePage');
	});
	$('.ActiveBtn').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/interactiveResponsePage');
	});
	$('.DisableBtn').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/interactiveResponseDisablePage');
	});
	...
	var btn_copyFunc = function(){
		...
		console.info('btn_copyFunc iMsgId:' + iMsgId);
 		window.location.replace(bcs.bcsContextPath + '/edit/interactiveResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Copy&from=interactiveResponsePage');
	};
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath +'/edit/getInteractiveMsgList?type=INTERACTIVE&status=ACTIVE&keywordInput=&pushDate='
		}).success(function(response){
			$('.dataTemplate').remove();
			...
			for(key in response.MsgMain){
				...
				msgData.find('.mainKeyword a').attr('href', bcs.bcsContextPath +'/edit/interactiveResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Edit&from=interactiveResponsePage');
				...
				msgData.find('.sendCount a').attr('href', bcs.bcsContextPath +'/edit/keywordAndInteractiveReportPage?iMsgId=' + iMsgId + '&userStatus=' + keyObj.userStatus+ '&reportType=Interactive');
