...
$(function(){
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath + '/market/richMenuSendGroupCreatePage');
	});
	
	var btn_copyFunc = function(){
		var groupId = $(this).attr('groupId');
		console.info('btn_copyFunc groupId:' + groupId);
 		window.location.replace(bcs.bcsContextPath + '/market/richMenuSendGroupCreatePage?groupId=' + groupId + '&actionType=Copy');
	};
	...
	var btn_deteleFunc = function(){
		...
		$.ajax({
			type : "DELETE",
			url : bcs.bcsContextPath + '/admin/deleteRichMenuSendGroup?groupId=' + groupId
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath + '/market/getRichMenuSendGroupList'
		}).success(function(response){
			$('.dataTemplate').remove();
			console.info(response);
