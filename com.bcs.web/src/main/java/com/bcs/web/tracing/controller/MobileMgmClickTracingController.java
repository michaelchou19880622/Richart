...
//            	return;
            ...
            Map<String, String> resultMap = LineLoginUtil.callRetrievingAPIfromMGM(code, UriHelper.getMgmClickOauth(), state);
...
//            } 
            ...
            Boolean friendFlag = Boolean.valueOf(resultMap.get("friendFlag"));
            logger.info("LineLoginUtil.callRetrievingAPI(code, UriHelper.getMgmOauth(), state) gefriendFlag:" +friendFlag);

            logger.info("lineUserService.findByMidAndCreateUnbind(uid)" );
            if(StringUtils.isNotBlank(uid)) {
                lineUserService.findByMidAndCreateUnbind(uid);
            }
            ...
            if(friendFlag){ // 好友
                ...
                session.setAttribute("MID", uid);
                logger.info("session.setAttribute MID :" + uid);
