...
//    private static Logger logger = Logger.getLogger(MobileMGMViewController.class);
	...
	public void goMgmPage(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		log.info("goMgmPage campaignId:" + campaignId);
...
//            String descriptionImgUrl = UriHelper.getResourceUri(ContentResource.RESOURCE_TYPE_IMAGE, shareCampaign.getDescriptionImgReferenceId());
	...
	@ResponseBody
	public String doMgmSharing(Model model, HttpServletRequest request, HttpServletResponse response) {
		log.info("doMgmSharing");
		...
		try {
			...
			log.info("[doMgmSharing] 1-1 find shareUserRecord = {}", shareUserRecord);
			...
			log.info("[doMgmSharing] 1-2 new shareUserRecord = {}", shareUserRecord);
			...
			String strSharedTime = simpleDateFormat.format(now);
			log.info("strSharedTime = {}", strSharedTime);
			
			String url = UriHelper.getMgmClickTracingUrl() + shareUserRecord.getShareUserRecordId() + "/" + strSharedTime;

			log.info("SC1:" + URLEncoder.encode(message + url, "UTF-8").replace("+", "%20"));
