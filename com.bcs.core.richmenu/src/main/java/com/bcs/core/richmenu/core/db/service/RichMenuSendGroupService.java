...
import com.bcs.core.richmenu.core.db.repository.RichMenuSendGroupRepository;
import com.bcs.core.richmenu.enums.RICH_MENU_DEFAULT_SEND_GROUP;
import com.bcs.core.utils.ObjectUtil;

@Service
public class RichMenuSendGroupService {
	/** Logger */
	private static Logger logger = LogManager.getLogger(RichMenuSendGroupService.class);
	
	public static final int pageSize = 1000; //PageSize Limit
	
	@Autowired
	private RichMenuSendGroupRepository sendGroupRepository;
	@Autowired
	private LineUserService lineUserService;

	public List<RichMenuSendGroup> findAll(){
	 ...
	 */
	public Map<Long, RichMenuSendGroup> generateDefaultGroupMap(){
		List<RichMenuSendGroup> groups = generateDefaultGroup();
		Map<Long, RichMenuSendGroup> result = new LinkedHashMap<Long, RichMenuSendGroup>(); 
		for(RichMenuSendGroup group : groups){
	 ...
	 */
	public List<RichMenuSendGroup> generateDefaultGroup(){
		List<RichMenuSendGroup> groups = new ArrayList<RichMenuSendGroup>();
	...
	public Map<Long, RichMenuSendGroup> findAllMap(){
		List<RichMenuSendGroup> groups = sendGroupRepository.findAll();
		Map<Long, RichMenuSendGroup> result = new HashMap<Long, RichMenuSendGroup>();
	...
	public Map<Long, String> findGroupTitleMap(){
		...
		List<RichMenuSendGroup> defaults = generateDefaultGroup();
		for(RichMenuSendGroup group : defaults){
	 ...
	 * @return SendGroup
	 */
	public RichMenuSendGroup findOne(Long groupId){
		if(groupId != null && groupId > 0){
			return sendGroupRepository.findOne(groupId);
		}
		else{
			Map<Long, RichMenuSendGroup> map = generateDefaultGroupMap();
	...
	public void save(RichMenuSendGroup sendGroup){
	...
	public void delete(Long groupId) throws BcsNoticeException{
		...
		RichMenuSendGroup sendGroup = sendGroupRepository.findOne(groupId);
