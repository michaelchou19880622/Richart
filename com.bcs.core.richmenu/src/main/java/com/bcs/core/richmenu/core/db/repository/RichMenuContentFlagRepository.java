...
public interface RichMenuContentFlagRepository extends EntityRepository<RichMenuContentFlag, Long> {

	@Transactional(readOnly = true, timeout = 30)
	@Query("select x.flagValue "
			+ "from RichMenuContentFlag x "
	...
	@Transactional(readOnly = true, timeout = 30)
	@Query("select x.referenceId, x.flagValue "
			+ "from RichMenuContentFlag x "
			+ "where x.referenceId in ( ?1 ) "
			+ "and x.contentType = ?2 "
			+ "order by x.flagValue asc")
	List<Object[]> findFlagValueByReferenceIdInAndContentTypeOrderByFlagValueAsc(List<String> referenceIds, String contentType);
