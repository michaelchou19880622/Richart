package com.bcs.core.richmenu.web.ui.service;
...
import com.bcs.core.richmenu.core.bot.scheduler.handler.ExecuteSendRichMenuTask;
...
import com.bcs.core.richmenu.core.db.entity.RichMenuSendGroup;
import com.bcs.core.richmenu.core.db.service.RichMenuSendGroupService;
import com.bcs.core.richmenu.web.ui.model.SendRichMenuDetailModel;
...
//			Date startTime = schedulerService.parseToDate(sendRichMenuModel.getSendingMsgTime());
//			Date now = new Date();
//			
//			// Validate Time
...
//			// Handle : SCHEDULE
//			// Validate ScheduleTime By Parse
//			String cronExpression = schedulerService.parseToCronExpression(sendRichMenuModel.getSendingMsgTime());
//			Long msgId = this.saveMessage(sendRichMenuModel, adminUserAccount, MsgMain.MESSAGE_STATUS_SCHEDULED);
...
//	}
	...
	public void saveDraftMessage(SendRichMenuModel SendRichMenuModel, String adminUserAccount) throws Exception{
		this.saveMessage(SendRichMenuModel, adminUserAccount, MsgMain.MESSAGE_STATUS_DRAFT);
	}
	 ...
	 */
	@Transactional(rollbackFor=Exception.class, timeout = 30)
	private Long saveMessage(SendRichMenuModel SendRichMenuModel, String adminUserAccount, String status) throws Exception{
		logger.info("saveMessage");
		MsgMain msgMain = new MsgMain();
		
		if(SendRichMenuModel.getMsgId() != null && SendRichMenuModel.getMsgId() > 0){
			if(checkMsgMainStatus(SendRichMenuModel.getMsgId(), MsgMain.MESSAGE_STATUS_DRAFT)){
				msgMain.setMsgId(SendRichMenuModel.getMsgId());
				// Remove Old Setting Detail
				msgDetailService.deleteByMsgIdAndMsgParentType(SendRichMenuModel.getMsgId(), MsgMain.THIS_PARENT_TYPE);
			}
		}
		msgMain.setGroupId(SendRichMenuModel.getSendGroupId());
		msgMain.setSerialId(SendRichMenuModel.getSerialId());
		msgMain.setSendType(SendRichMenuModel.getSendingMsgType());
		msgMain.setScheduleTime(SendRichMenuModel.getSendingMsgTime());
		msgMain.setStatus(status);
		msgMain.setModifyTime(new Date());
		msgMain.setModifyUser(adminUserAccount);
		msgMain.setMsgTag(contentFlagService.concat(SendRichMenuModel.getMsgTagList(), 50));
		...
		contentFlagService.save(
				...
				SendRichMenuModel.getMsgTagList());
		
		List<SendRichMenuDetailModel> list = SendRichMenuModel.getSendMsgDetails();
		this.createMsgDetail(msgMain.getMsgId(), list, adminUserAccount, MsgMain.THIS_PARENT_TYPE);
		
		return msgMain.getMsgId();
	}
	
	public void createMsgDetail(Long msgId, List<SendRichMenuDetailModel> list, String adminUserAccount, String parentType ) throws Exception{

		if(list != null && list.size() > 0){
			for(SendRichMenuDetailModel model : list){
	 ...
	 */
	@Transactional(rollbackFor=Exception.class, timeout = 30)
	public void redesignMsg(SendRichMenuModel SendRichMenuModel, String adminUserAccount) throws Exception{ 

		MsgMain msgMain = msgMainService.findOne(SendRichMenuModel.getMsgId());
		...
		if(msgMain != null){
			...
			logger.error("Redesign Msg Not Found MsgId:" + SendRichMenuModel.getMsgId());
			throw new BcsNoticeException("請選擇正確的訊息");
		}
	}

	private List<Message> validateMessagesWichMessage(List<SendRichMenuDetailModel> sendMsgDetails, String mid) throws Exception{
		return validateMessagesWichMessage(sendMsgDetails, mid, null);
	}
	
	private List<Message> validateMessagesWichMessage(List<SendRichMenuDetailModel> sendMsgDetails, String mid, Map<String, String> replaceParam) throws Exception{

		List<Message> result = new ArrayList<Message>();
		for(SendRichMenuDetailModel sendMsgDetail : sendMsgDetails){
	 ...
	 * @throws Exception
	 */
	private List<MsgGenerator> validateMessages(List<SendRichMenuDetailModel> sendMsgDetails) throws Exception{
	...
	public void setGroups(Map<String, Object> result){
		...
		try{
			Map<Long, String> groups = richMenuSendGroupService.findGroupTitleMap();
...
//	public void sendMsgToMid(String mid, List<SendRichMenuDetailModel> sendMsgDetails, String account) throws Exception{
...
//	}
//	
//	public void sendMsgToMid(String mid, List<SendRichMenuDetailModel> sendMsgDetails, String account, SendRichMenuModel SendRichMenuModel) throws Exception{
//		logger.info("sendMsgToMid:" + mid);
//		
//		List<String> mids = new ArrayList<String>();
//		mids.add(mid);
//		sendMsgToMids(mids, sendMsgDetails, account, SendRichMenuModel);
//	}
//	
//	public void sendMsgToMids(List<String> mids, List<SendRichMenuDetailModel> sendMsgDetails, String account, SendRichMenuModel SendRichMenuModel) throws Exception{
//		logger.info("sendMsgToMid:" + mids);
//
//		sendMsgToMids(mids, sendMsgDetails, false, null, account, SendRichMenuModel);
//	}
//	
//	public void sendMsgToMids(List<String> mids, List<SendRichMenuDetailModel> sendMsgDetails, boolean async, Long updateMsgId, String account, SendRichMenuModel SendRichMenuModel) throws Exception{
//		this.sendMsgToMids(mids, sendMsgDetails, async, updateMsgId, account, 0, SendRichMenuModel);
//	}
//	
//	public void sendMsgToMids(List<String> mids, List<SendRichMenuDetailModel> sendMsgDetails, boolean async, Long updateMsgId, String account, int retryCount, SendRichMenuModel SendRichMenuModel) throws Exception{
//		logger.info("sendMsgToMid:" + mids);
//		
//		if(mids != null && mids.size() > 0){
//			
...
//						
//						if(SendRichMenuModel != null && StringUtils.isNotBlank(SendRichMenuModel.getSerialId())){
//							replaceParam = serialSettingService.getSerialSettingReplaceParam(SendRichMenuModel.getSerialId(), mid);
...
//							midsRetry.add(mid);
//							this.sendMsgToMids(midsRetry, sendMsgDetails, async, updateMsgId, account, retryCount + 1, SendRichMenuModel);
//							break;
//						}
//						else{
//							throw e;
