...
public class MessageProcessService {
	...
	public JSONObject generateConfirmSwitchMsg(Integer type, Boolean isLeaveMessage) {
		...
		/* 留過言又觸發敏感詞的使用者的訊息文案中不應出現「轉接客服專員」 */
		if(key.equals(LIVE_CHAT_WORDING.SWITCH_MESSAGE_KEYWORD.toString()) && isLeaveMessage) {
			String delimeter = LiveChatWordingUtil.getString(LIVE_CHAT_WORDING.WORDING_SPLIT_KEY.toString());
			String[] partition = text.split(delimeter);
			
			logger.debug("[generateConfirmSwitchMsg] delimeter: " + delimeter);
			logger.debug("[generateConfirmSwitchMsg] partition[0]: " + partition[0]);
			logger.debug("[generateConfirmSwitchMsg] partition[1]: " + partition[1]);
			
			text = partition[0] + partition[1];
		}
